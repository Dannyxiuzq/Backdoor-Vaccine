...
def _setup_lora_tuning(
    ...
    if model_args.adapter_name_or_path is not None:
        ...
        if adapter_to_resume is not None:  # resume lora training
            if model_args.use_unsloth:
                ...
                if not (len(finetuning_args.lora_target) == 1 and finetuning_args.lora_target[0] == "all"):
                    target_modules = finetuning_args.lora_target
                    # 只让 up_proj / gate_proj 的 LoRA 参数可训练，其他 LoRA 参数冻结
                    for name, p in model.named_parameters():
                        if "lora_A" in name or "lora_B" in name:
                            # 依据模块路径名里是否含 up_proj/gate_proj 来筛选
                            if any(k in name for k in target_modules):
    ...
    if is_trainable and adapter_to_resume is None:  # create new lora weights while training
        if len(finetuning_args.lora_target) == 1 and finetuning_args.lora_target[0] == "all":
            ...
            logger.info(f"Found targeted linear modules: {finetuning_args.lora_target}")
            target_modules = finetuning_args.lora_target
